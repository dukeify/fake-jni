...
//  void FakeClass::* const field;
 ...
 //isStatic(true), name(std::move(name)), sig(generateSignature(field)), sField(field) {
 //safety third
 isStatic(true), name(std::move(name)), sig(generateSignature(field)), sField((void * const)field) {
  fields.push_back(this);
 }

 template<typename F, typename D>
 explicit _jfieldID(std::string name, F D::* field) :
 //isStatic(false), name(std::move(name)), sig(generateSignature(field)), field(static_cast<int FakeClass::* const>(field)) {
 //safety third
 isStatic(false), name(std::move(name)), sig(generateSignature(field)), field((int FakeClass::* const)field) {
  fields.push_back(this);
 }

 //TODO problem also here, the returned type needs to be stored in the case of instance members in order to
 template<typename F>
...
public:
 ...
 //TODO problem lies here, the return type needs to be R * in order to work with the JNI... how should returned values
 // be stored?
 template<typename R, typename... A>
 R invoke(FakeClass * const clazz, A... args) {
  if (isStatic) {
   ...
   return (clazz->*reinterpret_cast<MemberFuncRef<R, FakeClass, A...>>(func))(args...);
  }
 }
...
//NOTE Because of the singly linked ascending class hierarchy, there is no prevention against linking single methods
// or fields to multiple classes. This could lead to collisions where two separate classes define identical fields and
// the incorrect field is resolved on a depth-first-search. Sticking to the high-level API will prevent collisions,
// however, using the JNI will not. This needs to be fixed eventually! Requires a classloader implementation.
//
//NOTE Using references prevents users from having to allocate memory to create _jmethodIDs or _jfieldIDs. To simplify
// interactions with the JNI, the typedef'd pointer equivalents of classes are used internally.
...
public:
 ...
 explicit FakeClass(std::string fullyQualifiedName) : fullyQualifiedName(std::move(fullyQualifiedName)) {
  registerClass(this);
 }
...
// void setField(const _jfieldID &fid, F * const value) {
 ...
 static void registerClass(FakeClass *fakeClass) {
  if (isClassRegistered(fakeClass)) {
   throw std::runtime_error("This class has already been registered on the JVM!");
  }
...
//  assert(!clazz && "The jobject must inherit from FakeClass!");
 ...
 //Since no classloader model has been implemented, the current implementation of this function does not require
 //the target class to resolved
