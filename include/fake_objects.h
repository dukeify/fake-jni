...
template <typename R, typename... A> using FuncRef = R (*)(A...);
...
public:
  ...
  template <typename F> F* const getField(FakeClass* const clazz) {
    if (isStatic) {
  ...
  template <typename F> void setField(FakeClass* clazz, F value) {
    if (isStatic) {
  ...
  template <typename F> static std::string generateSignature(F* field);
...
template <typename F> std::string _jfieldID::generateSignature(F* field) {
  std::string sig;
